...
shaka.media.DrmEngine = class {
  ...
  onKeyStatusesChange_(event) {
    ...
    keyStatusMap.forEach((status, keyId) => {
      ...
      shaka.log.debug('Shaka player status ' + status);
      ...
      if (status == 'internal-error') {
        shaka.log.debug('Error thrown from player for Internal Error');
        this.onError_(new shaka.util.Error(
            shaka.util.Error.Severity.CRITICAL,
            shaka.util.Error.Category.DRM,
            shaka.util.Error.Code.RESTRICTIONS_CANNOT_BE_MET, keyIdHex));
      } else {
